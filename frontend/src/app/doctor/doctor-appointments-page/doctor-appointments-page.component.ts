...
export class DoctorAppointmentsPageComponent {
  constructor(public dialog: MatDialog, private doctorService: DoctorService, private toasterService: ToasterService, private userService: UserService) { }
  ...
  async getAppointmentsByDoctorId(userId: string): Promise<void> {
    const result = await this.doctorService.getAppointmentsByDoctorId(userId)
  ...
  async mapAppointmentsData(appointments: any): Promise<void> {
    appointments.forEach(async (appointment: any) => {
      const patient = await this.doctorService.getPatientById(appointment.patientId).toPromise().catch(error => error);
  ...
  async onApproveBtnClick(id: string, appointment: Appointment) {
    ...
    const result = await this.doctorService.updateAppointment(id, appointment).toPromise().catch(error => error);
